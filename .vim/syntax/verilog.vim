...
" macros fold
" ported from http://vim.wikia.com/wiki/VimTip1376
" define groups that cannot contain the start of a fold
syn cluster verilogNoFold contains=verilogComment,vimComment,vimLineComment,vimCommentString,vimString,vimSynKeyRegion,vimSynRegPat,vimPatRegion,vimMapLhs,vimOperParen,@EmbeddedScript
...
syn region verilogFoldIfContainer
      ...
      \ transparent
      \ keepend extend
      \ containedin=ALLBUT,@verilogNoFold
      \ contains=NONE
syn region verilogFoldIf
      \ start="\`ifdef\|\`ifndef"
      \ end="\`else\|\`elsif"ms=s-1,me=s-1
      \ skip=+"\%(\\"\|[^"]\)\{-}\%("\|$\)\|'[^']\{-}'\|\/\/.\{-}$\|\/\*\_.\{-}\*\/+
      \ fold transparent
      \ keepend
...
" syn region vDefFold start="\`ifdef\|\`ifndef" end="\`else\|\`endif" transparent fold  
" syn region vElseFold start="\`else" end="\`endif" transparent fold  
